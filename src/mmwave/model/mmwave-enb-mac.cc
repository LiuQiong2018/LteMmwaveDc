...
MmWaveEnbMac::DoDlHarqFeedback (DlHarqInfo params)
{
  NS_LOG_FUNCTION (this);
  // Update HARQ buffer
  std::map <uint16_t, MmWaveDlHarqProcessesBuffer_t>::iterator it =  m_miDlHarqProcessesPackets.find (params.m_rnti); // woody
		...
		if (slotAllocInfo.m_slotType != SlotAllocInfo::CTRL && slotAllocInfo.m_tddMode == SlotAllocInfo::DL)
		{
			...
			else
			{
				...
				if (dciElem.m_ndi == 1)
				{
					...
					std::map <uint16_t, MmWaveDlHarqProcessesBuffer_t>::iterator harqIt = m_miDlHarqProcessesPackets.find (rnti); // woody
				...
				else
				{
					...
					if (dciElem.m_tbSize > 0)
					{
						...
						std::map <uint16_t, MmWaveDlHarqProcessesBuffer_t>::iterator it = m_miDlHarqProcessesPackets.find (rnti); // woody
						NS_ASSERT(it!=m_miDlHarqProcessesPackets.end());
...
MmWaveEnbMac::DoAddUe (uint16_t rnti)
{
	...
	MmWaveDlHarqProcessesBuffer_t buf; // woody
	...
	m_miDlHarqProcessesPackets.insert (std::pair <uint16_t, MmWaveDlHarqProcessesBuffer_t> (rnti, buf)); // woody
