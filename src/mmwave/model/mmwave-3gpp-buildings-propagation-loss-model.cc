...
/*  if (Now().GetSeconds() < 2)
  ...
  else if (Now().GetSeconds() < 4)
  {
	if (stateChanged == 1)
	{
		NS_LOG_UNCOND(Now().GetSeconds() << " changed to " << txPowerDbm - GetLoss(a,b) - (double)variable - 32.0);
		stateChanged = 2;
	}
	return txPowerDbm - GetLoss (a,b) - (double)variable - 32.0;
  }
  else 
  {
	if (stateChanged == 2)
	{
		NS_LOG_UNCOND(Now().GetSeconds() << " changed to " << txPowerDbm - GetLoss(a,b) - (double)variable - 0);
		stateChanged = 3;
	}
	return txPowerDbm - GetLoss (a,b) - (double)variable;
  }*/
