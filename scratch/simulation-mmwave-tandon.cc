...
main (int argc, char *argv[])
{
        ...
        LogComponentEnable("PointToPointHelper", LOG_FUNCTION);
...
//	Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TcpCubic::GetTypeId ()));
	...
	NS_LOG_UNCOND("# Install obstacle blocks");
	Ptr < Building > building1a;
	building1a = Create<Building> ();
	building1a->SetBoundaries (Box (0.0,67.0,	0.0, 100,	0.0, 12));

	Ptr < Building > building1b;
	building1b = Create<Building> ();
	building1b->SetBoundaries (Box (10.0,57.0,	20.0, 90,	12.0, 95));

	Ptr < Building > building2;
	building2 = Create<Building> ();
	building2->SetBoundaries (Box (86.0,156.0,	18.0, 100,	0.0, 40));

	/*Ptr < Building > building3;
	building3 = Create<Building> ();
	building3->SetBoundaries (Box (178.0,240.0,	18.0, 100,	0.0, 40));*/

	Ptr < Building > building4a;
	building4a = Create<Building> ();
	building4a->SetBoundaries (Box (21.0,67.0,	119.0, 149,	0.0, 32));

	Ptr < Building > building4b;
	building4b = Create<Building> ();
	building4b->SetBoundaries (Box (38.0,67.0,	149.0, 199,	0.0, 32));

	Ptr < Building > building4c;
	building4c = Create<Building> ();
	building4c->SetBoundaries (Box (14.0,38.0,	149.0, 164,	0.0, 30));

	Ptr < Building > building4d;
	building4d = Create<Building> ();
	building4d->SetBoundaries (Box (0.0,14.0,	149.0, 185,	0.0, 32));

	/*Ptr < Building > building5a;
	building5a = Create<Building> ();
	building5a->SetBoundaries (Box (178.0,278.0,	119.0, 163,	0.0, 80));

	Ptr < Building > building5b;
	building5b = Create<Building> ();
	building5b->SetBoundaries (Box (203.0,258.0,	163.0, 184,	0.0, 30));

	Ptr < Building > building5c;
	building5c = Create<Building> ();
	building5c->SetBoundaries (Box (178.0,238.0,	184.0, 226,	0.0, 30));*/

	Ptr < Building > building6;
	building6 = Create<Building> ();
	building6->SetBoundaries (Box (91.0,156.0,	184.0, 221,	0.0, 17));

	/*Ptr < Building > building4;
	building4 = Create<Building> ();
	building4->SetBoundaries (Box (30.0,70.0, 	60.0, 80,	0.0, 20));*/

	Ptr < Building > tree1;
	tree1 = Create<Building> ();
	tree1->SetBoundaries (Box (100.0,100.50,	120.0, 120.50,	0.0, 3));

	Ptr < Building > tree1a;
	tree1a = Create<Building> ();
	tree1a->SetBoundaries (Box (95.0,105.50,	115.0, 125.50,	3, 12));

	Ptr < Building > tree2;
	tree2 = Create<Building> ();
	tree2->SetBoundaries (Box (140.0,140.50,	120.0, 120.50,	0.0, 2.5));

	Ptr < Building > tree2a;
	tree2a = Create<Building> ();
	tree2a->SetBoundaries (Box (134.0,146.50,	114.0, 126.50,	2.5, 15));

	Ptr < Building > tree3;
	tree3 = Create<Building> ();
	tree3->SetBoundaries (Box (135.0,135.50,	132.0, 132.5,	0.0, 2));

	Ptr < Building > tree3a;
	tree3a = Create<Building> ();
	tree3a->SetBoundaries (Box (130.0,140.50,	127.0, 137.5,	2, 13));

	Ptr < Building > tree4;
	tree4 = Create<Building> ();
	tree4->SetBoundaries (Box (110.0,110.50,	160.0, 160.5,	0.0, 3));

	Ptr < Building > tree4a;
	tree4a = Create<Building> ();
	tree4a->SetBoundaries (Box (105,115.50,		155.0, 165.5,	3, 20));
	...
	enbPositionAlloc->Add (Vector (160.0, 180.0, hBS));
